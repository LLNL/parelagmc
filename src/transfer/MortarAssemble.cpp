...
void MakeCompositeQuadrature2D(const DenseMatrix &polygon, 
    const double weight, const int order, IntegrationRule &c_ir)
{
    ...
    double triangle[3*2] = 
        { polygon.Data()[0], polygon.Data()[1], 0., 0., 0., 0. };
    ...
    for(int i = 2; i < polygon.Width(); ++i) 
    {
        ...
        const double scale = 
            fabs(isector.polygon_area_2(3, triangle)) / ( weight );
    ...
    const double weight, const int order, IntegrationRule &c_ir)
{
    ...
    isector.row_average( polyhedron.n_nodes, polyhedron.n_dims, 
        polyhedron.points, barycenter_p);

    const int max_n_sub_els = isector.max_n_elements_from_facets(polyhedron);
    const int max_n_sub_inc = 
        std::max(1, MAX_QUAD_POINTS / (max_n_sub_els * ir.Size()));
...
bool Intersect3D(const Mesh &m1, const int el1, const Mesh &m2, 
