...
bool MortarAssembler::Assemble(std::shared_ptr<SparseMatrix> &B)
{
    ...
    //////////////////////////////////////////////////
    int skip_zeros = 1;
    B = std::make_shared<SparseMatrix>(slave_->GetNDofs(), master_->GetNDofs());
    ...
    for(auto it = begin(pairs); it != end(pairs);    ) 
    {
        ...
        if(pair_intersected) 
        {
            ...
            for(auto i_ptr : integrators_) 
            {
                if(first) 
                {
                    i_ptr->AssembleElementMatrix(master_fe, master_ir, 
                        master_Trans, slave_fe, slave_ir, 
                        slave_Trans, cumulative_elemmat);
                    first = false;
                } 
                else 
                {
            ...
            B->AddSubMatrix(slave_vdofs, master_vdofs, cumulative_elemmat, 
                skip_zeros);
            intersected = true;
        }
    } 
    ...
    if(verbose) 
    {
        std::cout << "local_element_matrices_sum: " << 
            local_element_matrices_sum << std::endl;
        std::cout << "intersection volume: " << 
            total_intersection_volume << std::endl;
        std::cout << "B in R^(" << B->Height() <<  
            " x " << B->Width() << ")" << std::endl;
    }
...
bool MortarAssembler::Transfer(GridFunction &src_fun, GridFunction &dest_fun, bool is_vector_fe)
{
    ...
    std::shared_ptr<SparseMatrix> B = nullptr;
    if(!Assemble(B)) 
        return false;
