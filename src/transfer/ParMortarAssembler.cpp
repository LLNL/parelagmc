...
{
public:
    typedef parelagmc::BoxAdapter StaticBound;
...
public:
    ...
    typedef parelagmc::BoxBoxAdapter<Dimension> Bound;
    typedef parelagmc::ElementAdapter<Dimension> DataType;
...
{
public:
    typedef parelagmc::TreeTraits<Dimension> Traits;
...
static void write_element_selection(const Iterator &begin, 
    const Iterator &end, const Spaces &spaces, moonolith::OutputStream &os)
{
...
static void read_space(moonolith::InputStream &is, std::shared_ptr<FiniteElementSpace> &space, std::vector<ElementDofMap> &dof_map)
{
    ...
    space = std::make_shared<FiniteElementSpace>(mesh_ptr, fe_coll);
}
    ...
    const moonolith::SearchSettings &settings) 
{
    ...
    typedef parelagmc::MFEMTree<Dimensions> NTreeT;
    ...
    if (comm.is_root()) {
        std::cout << "-- L2Projector: n_intersections: "   		<< n_collection[0]
    ...
    const moonolith::SearchSettings &settings) 
{
        ...
        const ElementAdapter<Dimensions> &slave) -> bool 
    {
        ...
        {		
            ...
            if(Intersect2D(src_pts, dest_pts, intersection2)) 
            {
                total_intersection_volume += 
                    fabs(isector.polygon_area_2(intersection2.Width(), intersection2.Data()));
        ...
        if(pair_intersected) 
        {
            ...
            for(auto i_ptr : integrators) 
            {
                if(first) 
                {
                    i_ptr->AssembleElementMatrix(src_fe, src_ir, src_Trans, 
                        dest_fe, dest_ir, dest_Trans, cumulative_elemmat);
                    first = false;
                } 
                else 
                {
                    i_ptr->AssembleElementMatrix(src_fe, src_ir, src_Trans, 
                        dest_fe, dest_ir, dest_Trans, elemmat);
    ...
    if(comm.is_root()) 
        std::cout << "-- L2Projector: sum(B): " << volumes[0] << ", vol(I): " << volumes[1] << std::endl;
...
bool ParMortarAssembler::Assemble(std::shared_ptr<HypreParMatrix> &pmat)
{
    ...
    if(master_->GetMesh()->Dimension() == 2) 
        return parelagmc::Assemble<2>(comm, master_, slave_, integrators_, pmat, settings);

    if(master_->GetMesh()->Dimension() == 3) 
        return parelagmc::Assemble<3>(comm, master_, slave_, integrators_, pmat, settings);
...
bool ParMortarAssembler::Transfer(ParGridFunction &src_fun, ParGridFunction &dest_fun, bool is_vector_fe)
{
    ...
    std::shared_ptr<HypreParMatrix> B = nullptr;
    ...
    std::shared_ptr<HypreParMatrix> Dptr(b_form.ParallelAssemble());
