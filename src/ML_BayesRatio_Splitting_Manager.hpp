...
#ifndef ML_BAYESRATIO_SPLITTING_MANAGER_HPP_
#define ML_BAYESRATIO_SPLITTING_MANAGER_HPP_
...
namespace parelagmc
{
/// \class ML_BayesRatio_Splitting_Manager 
...
/// Splitting-E_post[Q]=E[R/Z] (divide, then subtract i.e. sum(r/z - r_c/z_c)). 
///
...
class ML_BayesRatio_Splitting_Manager : public Sampling_Method_Manager
{
public:
    /// Constructor
    ML_BayesRatio_Splitting_Manager(MPI_Comm comm, 
        const int nlevels, 
        BayesianInverseProblem & problem,
        parelag::ParameterList& params);

    /// Destructor
    virtual ~ML_BayesRatio_Splitting_Manager() = default;

    /// @{ \brief All special constructors and assignment are deleted.         
    ML_BayesRatio_Splitting_Manager(ML_BayesRatio_Splitting_Manager const&) = delete;    
    ML_BayesRatio_Splitting_Manager(ML_BayesRatio_Splitting_Manager&&) = delete;         
    ML_BayesRatio_Splitting_Manager&                                                
        operator=(ML_BayesRatio_Splitting_Manager const&) = delete;                 
    ML_BayesRatio_Splitting_Manager&                                                
        operator=(ML_BayesRatio_Splitting_Manager&&) = delete;                      
    ///@}            
...
private:
    ...
    double y=0., y_r=0., y_z=0., r=0., rc=0., z=0., zc=0., q=0., qc=0., c=0.;
    double c_tot=0.;
};

ML_BayesRatio_Splitting_Manager::ML_BayesRatio_Splitting_Manager(MPI_Comm comm_, 
            const int nlevels_, 
...
void ML_BayesRatio_Splitting_Manager::InitRun(
        std::vector<int> & level_nsamples_init)
{
    ...
    if(level_nsamples.Max() == 0 && pid == 0)
        logger << "%" << std::setw(13) << "level " << std::setw(14) << "R(xi) " 
                ...
                << std::setw(14) << "Y_Z(xi) " <<std::setw(14) << "Ratio(xi)"
                << std::setw(14) << "Y_Ratio(xi) " << std::setw(14) << "c \n";
    //Solve on the coarsest level
    {
        ...
        for(int isample(0); isample < nsamples; ++isample)
        {
            ...
            sums(ilevel, Ratio ) += q;
            sums(ilevel, ABS_Ratio ) += fabs(q);
            sums(ilevel, Ratio2) += q*q;
            sums(ilevel, YRatio ) += q;
            sums(ilevel, ABS_YRatio ) += fabs(q);
            sums(ilevel, YRatio2) += q*q;
            ...
            if(pid == 0)
                logger << std::setw(14) << ilevel << std::setw(14) << r << 
                    ...
                    std::setw(14) << z << std::setw(14) << q << 
                    std::setw(14) << q << std::setw(14) << c_tot << 
                    std::setw(14) << "\n"; 
        }
    ...
    for(int ilevel(nlevels-2); ilevel >= 0; --ilevel)
    {
        ...
        for(int isample(0); isample < nsamples; ++isample)
        {
            ...
            sums(ilevel, ABS_Ratio ) += fabs(q);
            sums(ilevel, Ratio2) += q*q;
            sums(ilevel, YRatio ) += y;
            sums(ilevel, ABS_YRatio ) += fabs(y);
            sums(ilevel, YRatio2) += y*y;
            ...
            if(pid == 0)
                logger << std::setw(14) << ilevel << std::setw(14) << r << 
                    ...
                    std::setw(14) << y_z << std::setw(14) << q << 
                    std::setw(14) << y << std::setw(14) << c << 
                    std::setw(14) << "\n"; 
        }
        level_nsamples[ilevel] += nsamples;
    }
...
void ML_BayesRatio_Splitting_Manager::Run()
{
    expectations   = 0.;
    sums           = 0.;
    eR             = 0.;
    ...
    if (!pid)
        std::cout << "FINAL ML_BayesRatio_Splitting_Manager ERRORS" << std::endl;
    ShowMe();
}

void ML_BayesRatio_Splitting_Manager::ShowMe(std::ostream & os)
{
    int total_width = 79;
    int name_width = 40;
    ...
    if(pid == 0)
    {
        ...
        os << "ML_BayesRatio_Splitting_Manager Errors: " << std::endl
           ...
           << std::setw(18) << std::left << eYRatio.Sum() << "\n\n"
        ...
        os << "\n";
        os << std::setw(name_width+2) << std::left << "Alpha"
        ...
        os << std::setw(name_width+2) << std::left << "E[Ratio] "
            << std::setw(2) << std::left;
        eRatio.Print(std::cout);
        os << std::setw(name_width+2) << std::left << "E[|Ratio|] "
            << std::setw(2) << std::left;
        eABS_Ratio.Print(std::cout);
        os << std::setw(name_width+2) << std::left << "Var[Ratio] "
            << std::setw(2) << std::left;
        varRatio.Print(std::cout);
        os << std::setw(name_width+2) << std::left << "E[Y_Ratio] "
            << std::setw(2) << std::left;
        eYRatio.Print(std::cout);
        os << std::setw(name_width+2) << std::left << "E[|Y_Ratio|] "
            << std::setw(2) << std::left;
        eABS_YRatio.Print(std::cout);
        os << std::setw(name_width+2) << std::left << "Var[Y_Ratio] "
            << std::setw(2) << std::left;
        varYRatio.Print(std::cout);
        os << std::string(total_width, '=') << std::endl;
   }
}

//----------------------------------------------------------------------------------------------------------//
void ML_BayesRatio_Splitting_Manager::computeNSamplesMSE()
{
    ...
    expectations.GetColumn(YRatio, eYRatio);
    expectations.GetColumn(YRatio2, varYRatio);
    expectations.GetColumn(ABS_YRatio, eABS_YRatio);
    ...
    for(int ilevel(0); ilevel < nlevels; ++ilevel)
    {
        ...
        varRatio(ilevel) -= eRatio(ilevel)*eRatio(ilevel);
        varRatio(ilevel) *= static_cast<double>(level_nsamples[ilevel]) 
            / static_cast<double>(level_nsamples[ilevel] - 1);
        varYRatio(ilevel) -= eYRatio(ilevel)*eYRatio(ilevel);
        varYRatio(ilevel) *= static_cast<double>(level_nsamples[ilevel]) 
            / static_cast<double>(level_nsamples[ilevel] - 1);
    }
    ...
    alpha    = expWRegression(eYRatio, M,1);
    alphaABS = expWRegression(eABS_YRatio, M,1);
    beta     = expWRegression(varYRatio, M,1);
    ...
    else
    {
        ...
        if(nlevels > 3)
            expected_discretization_error2 = std::max(
                pow(m, 2.*alphaABS) * eABS_YRatio(1)*eABS_YRatio(1), eABS_YRatio(0)*eABS_YRatio(0))
                / ( pow(pow(m, -2.*alphaABS) - 1.,2));
        else if( nlevels ==3 )
            expected_discretization_error2 = (eABS_YRatio(0)*eABS_YRatio(0)) /
    ...
    for(int ilevel(0); ilevel < nlevels; ++ilevel)
        ml_estimator_variance += varYRatio(ilevel)/
                static_cast<double>(level_nsamples[ilevel]);
    ...
    double prop = 0.;
    for(int i(0); i < nlevels; ++i)
        prop += sqrt( varYRatio(i) * cost(i) );
    prop /= ratio*eps2;

    for(int i(0); i < nlevels; ++i)
    {
        double missings = prop * sqrt( varYRatio(i) / cost(i) );
...
#endif /* ML_BAYESRATIO_SPLITTING_MANAGER_HPP_ */
