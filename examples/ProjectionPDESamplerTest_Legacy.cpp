...
// system using hypre's ADS preconditioner.  
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    {
        std::cout << '\n' << std::string(50,'*') << '\n'
                 << "*  L2ProjectionPDESampler_Legacy.exe \n"
    ...
    if (myid == 0) std::cout << "-- L2ProjectionPDESampler" << std::endl;
    L2ProjectionPDESampler_Legacy projsampler(pmesh, pembedmesh, 
            dist, *master_list);
    if (myid == 0) std::cout << "-- BuildHierarchy: proj" << std::endl;
    {
    ...
    if (visualize) projsampler.SaveMeshGLVis("mesh");
    ...
    for (int ilevel = 0; ilevel < nLevels; ilevel++)
    {
        ...
        if (visualize) projsampler.SaveFieldGLVis(ilevel, proj_coef, "proj_legacy_realization");
    }
    ...
    for(int ilevel(0); ilevel < nLevels; ++ilevel)
    {
        ...
        for(int i(0); i < nsamples; ++i)
        {
            ...
            iters[ilevel] += projsampler.GetNumIters();
        }
        ...
        iters[ilevel] *= 1./static_cast<double>(nsamples);
    ...
    ReduceAndOutputBothInfo(ndofs_l, ndofs_g,
            stoch_size_l, stoch_size_g, nnz, iters);
