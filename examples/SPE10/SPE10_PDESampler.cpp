...
// Computes realizations of the SPDE sampler and saves GLVis figures for u, s           
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    if (unstructured)
        BuildTopologyAlgebraic(pmesh, coarsening_factor,
            topology);
    else
        BuildTopologyGeometric(pmesh, level_nElements, 
            topology);
    ...
    if (visualize)
    {
        // Save gf to visualize agglomerated elements
    ...
    for (int ilevel = 0; ilevel < nLevels; ilevel++)
    {
        ...
        sampler.Eval(ilevel, xi, s, u);
        sampler.SaveFieldGLVis(ilevel, s, "realization_s");
        sampler.SaveFieldGLVis_u(ilevel, u, "realization_u");
