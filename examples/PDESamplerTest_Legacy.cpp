...
int main (int argc, char *argv[])
{
  ...
  try 
  {
    ...
    for (int ilevel = 0; ilevel < nLevels; ilevel++)
    {
        ...
        if (visualize) sampler.SaveFieldGLVis(ilevel, coef, 
            "pde_legacy_realization");
    }
    ...
    for(int ilevel(0); ilevel < nLevels; ++ilevel)
    {
        ...
        if (!myid && verbose) std::cout << "Level " << ilevel << ":\n";
        {
            ...
            for(int i(0); i < nsamples; ++i)
            {
                ...
                iters[ilevel] += sampler.GetNumIters();
            }
        }
        ...
        if (visualize) 
        {
            sampler.SaveFieldGLVis(ilevel, expectation, 
                "pde_legacy_expectation");
            sampler.SaveFieldGLVis(ilevel, chi_cov, 
                "pde_legacy_cov_chi");
            sampler.SaveFieldGLVis(ilevel, marginal_variance, 
                "pde_legacy_marginal_variance");
        }
