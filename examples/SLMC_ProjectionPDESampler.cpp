...
// input coefficient realized using the SPDE sampler with 
// non-matching mesh embedding. 
...
int main (int argc, char *argv[])
{
  ...
  try
  {
   ...
   // Boundary conditions and QoI specifications
    ...
    {
        std::cout << '\n' << std::string(50,'*') << '\n'
                 << "*  Standard Monte Carlo simulation with SPDE sampler with L2Projection \n"
    ...
    if (!myid && verbose) std::cout << "-- Construct DarcySolver\n";
    ...
    if (!myid && verbose) std::cout << "-- Darcy BuildHierarchy" << std::endl;
    {
    ...
    if (!myid && verbose) 
        std::cout << "-- Construct L2ProjectionPDESampler" << std::endl;
    ...
    if (!myid && verbose) std::cout << "-- SLMC Run" << std::endl;
    manager.Run();
