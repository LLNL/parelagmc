...
// permeability realizations computed with either a truncated KL expansion using 
// an AnalyticExponentialCovariance or Matern covariance, or the SPDE
// sampler (without mesh embedding). 
// 
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    {
        std::cout << '\n' << std::string(50,'*') << '\n'
                << "*  MLMC simulation with either analytic, matern or PDE sampler \n"
    ...
    if (!myid) std::cout << "-- Agglomerating the meshe " << std::endl;
    {                                                                          
    ...
    {
        if (myid == 0) std::cout << "-- Building AnalyticExponentialCovariance " << std::endl;
    ...
    {
        if (myid == 0) std::cout << "-- Building MaternCovariance " << std::endl;
        cov_func = make_unique<MaternCovariance>(pmesh, *master_list);
        sampler = make_unique<KLSampler>(pmesh, dist, *cov_func, *master_list);
    }
    else // PDE
    {
        if (myid == 0) std::cout << "-- Building PDESampler " << std::endl;
    ...
    if (myid == 0) std::cout << "-- MLMC Run" << std::endl;
    manager.Run();
