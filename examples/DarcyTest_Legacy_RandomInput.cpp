...
// Solve a mixed Darcy problem using MINRES with a L2-H1 preconditioner  
// with a random permeability coefficient generated with the SPDE sampler
// with non-matching mesh embedding (using L2ProjectionPDESampler_Legacy
// which uses hypre's ADS preconditioning).
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    {
        std::cout << '\n' << std::string(50,'*') << '\n'
                 << "*  DarcySolver_Legacy example with random input from L2ProjectionPDESampler\n"
    ...
    std::unique_ptr<DarcySolver_Legacy> solver =
        make_unique<DarcySolver_Legacy>(pmesh, *master_list);
    ...
    if (!myid) std::cout << "-- Construct L2ProjectionPDESampler_Legacy" << std::endl;
    
    auto sampler = make_unique<L2ProjectionPDESampler_Legacy>
    ...
    {
        {
            Timer timer = TimeManager::AddTimer(
                std::string("Darcy (Legacy): SolveFwd -- Level ")
                .append(std::to_string(i)));
        }
        {
            Timer timer = TimeManager::AddTimer(
                std::string("Sample Generation (Legacy) -- Level ")
                .append(std::to_string(i)));
        }
        {
            Timer timer = TimeManager::AddTimer(
                std::string("MC Sample (Legacy) -- Level ")
                .append(std::to_string(i)));
        }
    }
    ...
    Vector darcy_iters(nLevels);
    Vector sampler_iters(nLevels);
        ...
        {
            {
                ...
                if (!myid && verbose) std::cout << "\nSampler:\n";
                {
                    ...
                    sampler_iters[ilevel] += sampler->GetNumIters();
                }
                if (!myid && verbose) std::cout << "Darcy:\n";
                {
                    ...
                    darcy_iters[ilevel] += solver->GetNumIters();
                }    
                EQ += Q;
            }
        }
    ...
    for (int ilevel = 0; ilevel < nLevels; ++ilevel)
    {
        ...
        sampler_nnz[ilevel] = sampler->GetNNZ(ilevel);
        ...
        darcy_iters[ilevel] *= 1./static_cast<double>(nsamples);
        sampler_iters[ilevel] *= 1./static_cast<double>(nsamples);
    }

    // Generate some pictures: realization, then DarcySolver_Legacy will 
    // save u,p with uncertain coef
    if (visualize)
    {
        if (myid == 0) 
        {
    ...
    ReduceAndOutputDofsInfo(ndofs_l, ndofs_g, darcy_nnz, darcy_iters);
    ReduceAndOutputStochInfo(stoch_size_l, stoch_size_g, sampler_nnz, sampler_iters);
