...
#include "ML_BayesRatio_Splitting_Manager.hpp"
#include "ML_BayesRatio_Manager.hpp"
...
// If "Use splitting manager"=true, the 'ML_BayesRatio_Splitting_Manager' 
// is used to compute Splitting-E_post[Q]=E[R/Z] (divide, then subtract 
// i.e. sum(r/z - r_c/z_c)), otherwise the 'ML_BayesRatio_Manager' computes 
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    if (use_splitting_manager)
        manager = make_unique<ML_BayesRatio_Splitting_Manager>(
            comm, nLevels, *bayesian_problem, *master_list);
    else
        manager = make_unique<ML_BayesRatio_Manager>(
            comm, nLevels, *bayesian_problem, *master_list);
