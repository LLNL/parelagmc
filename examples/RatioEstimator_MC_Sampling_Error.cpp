...
// variety of number of samples with a fixed spatial discretization to 
// examine the sampling error. 
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    std::vector<double> v_qoi_pw_data_point = prob_list.Get(                   
        "PW pressure QoI spatial point", std::vector<double>{0.5, 0.5});       
    ...
    {
        std::cout << '\n' << std::string(50,'*') << '\n'
                 << "*  RatioEstimator_MC_Sampling_Error.exe \n"
                 ...
                 << "*  Initial number of Samples: " << nsamples << '\n';
        if (unstructured)
        {
    ...
    Vector ndofs_l(1);
    Vector ndofs_g(1);
    Vector stoch_size_l(1);
    Vector stoch_size_g(1);
    
    double c_tot=0.;
    double r=0., r2=0., z=0., z2=0.;
    
    int ilevel = 0;
    double R, Z, c, exp_r, exp_z, form_var_r, form_var_z;
    Vector zxi, xi, zcoef, coef; 

    if (!myid) std::cout << "NumSamples" << std::setw(10) << "E[R] "                    
                << std::setw(12) << "Var[R] " << std::setw(12) << "E[Z] "      
                << std::setw(12) << "Var[Z] " << std::setw(12) << "E[Q] \n";      

    // Solve on fine (level=1) level (where number of samples in increasing)
    int current_nsamples = 0;
    for (int n = 0; n < 10; n++) 
    {
        for (int i = n*nsamples; i < (n+1)*nsamples; i++)
        {
            // Compute Z
        ...
        const double ns = static_cast<double>(current_nsamples);
        
        // Compute variance
        exp_r = r/ns;
        exp_z = z/ns; 
        
        form_var_r = ns*(r2/ns - exp_r*exp_r)/(ns - 1.);
        form_var_z = ns*(z2/ns - exp_z*exp_z)/(ns - 1.);
        ...
        if (!myid) std::cout << current_nsamples << std::setw(18) << exp_r                          
            << std::setw(12) << form_var_r << std::setw(12) << exp_z           
            << std::setw(12) << form_var_z << std::setw(12) << exp_r/exp_z << '\n';         
    }
