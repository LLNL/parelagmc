...
// Computes various statistics and realizations of SPDE sampler
// with matching mesh embedding using CG preconditioned with 
// hypre's ADS. 
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    {
        std::cout << '\n' << std::string(50,'*') << '\n'
                 << "*  EmbeddedPDESamplerTest_Legacy.exe \n"
    ...
    for (int ilevel = 0; ilevel < nLevels; ilevel++)
    {
        ...
        if (visualize) embedsampler.SaveFieldGLVis(ilevel, embedcoef, 
            "embed_legacy_realization");
    }
    ...
    for(int ilevel(0); ilevel < nLevels; ++ilevel)
    {
        int embed_s_size = embedsampler.EmbedSampleSize(ilevel);
        Vector embed_expectation(embed_s_size);
        Vector embed_chi_cov(embed_s_size);
        Vector embed_marginal_variance(embed_s_size);
        ...
        Vector & embed_chi_level(*embed_chi[ilevel]);
        {
            ...
            for(int i(0); i < nsamples; ++i)
            {
                ...
                embedsampler.EmbedEval(ilevel, xi, embedcoef);
                double embed_chi_coef = dot(embedcoef, embed_chi_level, comm);
                embed_chi_cov.Add(embed_chi_coef, embedcoef);
                embed_expectation.Add(1., embedcoef);
                for(int k = 0; k < embed_s_size; ++k)
                    embed_marginal_variance(k) += embedcoef(k)*embedcoef(k);
                iters[ilevel] += embedsampler.GetNumIters();
            }
        }
        iters[ilevel] /= static_cast<double>(nsamples);
        ndofs_l[ilevel] = embedsampler.SampleSize(ilevel);
        ndofs_g[ilevel] = embedsampler.GlobalSampleSize(ilevel);   
