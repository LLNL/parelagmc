...
// Computes the ratio estimator of the Bayesian posterior          
// expectation of a QoI (eff_perm) using MLMC estimators            
// for a fixed number of samples. It computes                                  
// E_post[Q] = E[R]/E[Z] (subtract, then divide i.e. sum(r-rc)/sum(z-zc)),                                   
// and Splitting-E_post[Q]=E[R/Z] (divide, then subtract i.e. sum(r/z - r_c/z_c))                           
...
int main (int argc, char *argv[])
{
  ...
  try
  {
    ...
    {
        std::cout << '\n' << std::string(50,'*') << '\n'
                 << "*  RatioEstimator_MLMC.exe \n"
                 << "*  XML filename: " << xml_file << "\n*\n"
                 << "*  Sampler Name: "; 
        if (sampler_name.compare("analytic") == 0)
            std::cout << " KL with analytic covariance " << std::endl;
        else
            std::cout << " PDE sampler with mesh embedding (L2Projection) " << std::endl;
    ...
    if (!myid) std::cout << "-- Construct Sampler" << std::endl;
    std::unique_ptr<MLSampler> sampler;
    if (sampler_name.compare("analytic") == 0)
    {
        AnalyticExponentialCovariance cov_func(                 
                pmesh, *master_list);        
    ...
    {
        sampler = make_unique<L2ProjectionPDESampler>
    ...
    Vector zxi, xi, coef, zcoef;
    double c;
   
    double R, Z, Rc, Zc; 
    double c_tot=0.;
    double r=0., r2=0., z=0., z2=0.;
    double ry=0., ry2=0., zy=0., zy2=0.;
    double sum_r=0., sum_z=0.; 
    double splitting_ratio=0., ratio_diff=0.; 
   

    if (!myid) std::cout << "L" << std::setw(10) << "E[R] "                       
        << std::setw(12) << "Var[R] " << std::setw(12) << "E[Y_R] "      
        << std::setw(12) << "Var[Y_R] " << std::setw(12) << "E[Z] "      
        << std::setw(12) << "Var[Z] " << std::setw(12) << "E[Y_Z] "
        << std::setw(12) << "Var[Y_Z] " << std::setw(12) << "E[Q] \n";
     
    // Solve on coarsest level: L-1 
    {
        int ilevel = nLevels-1;
        {
            ...
            for (int i = 0; i < v_nsamples[ilevel]; i++)
            {
                ...
                //std::cout << "Z_0 " << Z << " R_0 " << R << std::endl;
                ry += R; r += R; ry2 += R*R; r2 += R*R; 
                zy += Z; z += Z; zy2 += Z*Z; z2 += Z*Z; 
                ratio_diff += R/Z;
            }
        }
        double ns = static_cast<double>(v_nsamples[ilevel]);
        ry /= ns;
        r /= ns;
        ry2 /= ns;
        r2 /= ns;
        ...
        sum_r += ry;
        sum_z += zy; 
        splitting_ratio += ratio_diff;

        // Compute variance 
        double form_var_r = 0.0, form_var_ry = 0.0;
        form_var_ry = ns*(ry2 - ry*ry)/(ns - 1.);
        form_var_r = ns*(r2 - r*r)/(ns - 1.);

        double form_var_z = 0.0, form_var_zy = 0.0;
        form_var_zy = ns*(zy2 - zy*zy)/(ns - 1.);
        form_var_z = ns*(z2 - z*z)/(ns - 1.);
        ...
        if (!myid) std::cout << ilevel << std::setw(10) << r                       
            << std::setw(12) << form_var_r << std::setw(12) << ry      
            << std::setw(12) << form_var_ry << std::setw(12) << z      
            << std::setw(12) << form_var_z << std::setw(12) << zy 
            << std::setw(12) << form_var_zy << std::setw(12) << sum_r/sum_z <<"\n";
    }
    ...
    for (int ilevel = nLevels - 2; ilevel >= 0; --ilevel)
    {
        ...
        zy = 0., z = 0., zy2 = 0., z2 = 0.;
        {
            ...
            for (int i = 0; i < v_nsamples[ilevel]; i++)
            {
                ...
                // Compute Z_c
                bayesian_problem->EvalPrior(ilevel+1, zxi, zcoef);
                bayesian_problem->ComputeLikelihood(ilevel+1, zcoef, Zc, c); c_tot += c;
                
                // Compute R_c = Qc*pi_like_c
                if (independent)
                    bayesian_problem->EvalPrior(ilevel+1, xi, coef); 
                else
                    coef = zcoef;
                bayesian_problem->ComputeR(ilevel+1, coef, Rc, c); c_tot += c;
                ry += R - Rc;
                r += R;
                ry2 += (R - Rc)*(R - Rc);
                r2 += R*R;
    ...
    if (!myid) std::cout << "\nE[Q]           " << sum_r/sum_z << std::endl; 
    if (!myid) std::cout <<   "Splitting E[Q] " << splitting_ratio << "\n" << std::endl; 
